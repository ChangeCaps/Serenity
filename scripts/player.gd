...
func _physics_process(delta: float) -> void: 
  ...
  var movement := Vector3(movement_input.x, 0.0, movement_input.y);
  movement = head.global_transform.basis * movement.normalized();

  var horizontal_velocity := Vector3(velocity.x, 0.0, velocity.z);
  var current_speed := movement.dot(horizontal_velocity); 

  if is_grounded():
    var delta_speed: float;
    if is_sliding:
      delta_speed = clamp(slide_speed - current_speed, 0.0, slide_acceleration * delta);
    else:
      delta_speed = clamp(walk_speed - current_speed, 0.0, walk_acceleration * delta);

    velocity += movement * delta_speed;
    velocity -= horizontal_velocity * current_friction() * delta;
  else:
    var delta_speed: float = clamp(air_speed - current_speed, 0.0, air_acceleration * delta);
    velocity += movement * delta_speed;
...
func _process(delta: float) -> void:
  ...
  # fov
  var clamped_speed = clamp(velocity.length(), 0.5, walk_speed * 4.0);
